#include "Engine3D/Rendering/HybridRenderer3D.h"
...
void HybridRenderer3D::beginFrame() {
    ...
    std::cout << "Начало гибридного кадра (качество: " << qualityAdjustmentFactor << ")" << std::endl;
}

void HybridRenderer3D::endFrame() {
    auto endTime = std::chrono::high_resolution_clock::now();
    auto duration = std::chrono::duration_cast<std::chrono::microseconds>(endTime - std::chrono::high_resolution_clock::now());
...
void HybridRenderer3D::renderScene(const std::vector<std::shared_ptr<GaussianField3D>>& gaussianFields,
                                  const std::vector<std::shared_ptr<Mesh3D>>& meshes,
                                  const std::vector<Math::Matrix4>& transforms) {
    
    std::cout << "Рендеринг гибридной сцены: " 
              << gaussianFields.size() << " полей гауссианов, "
    ...
    if (this->isRenderPassEnabled(RenderPass::DEPTH_PREPASS) && renderSettings.enableDepthPrepass) {
        hybridStats.rasterizationTime += this->measurePassTime([&]() {
            this->executeDepthPrepass(meshes, transforms);
        });
    }
    
    // Этап 2: Gaussian Rasterization
    if (this->isRenderPassEnabled(RenderPass::GAUSSIAN_RASTERIZATION) && renderSettings.enableGaussianSplatting) {
        hybridStats.rasterizationTime += this->measurePassTime([&]() {
            this->executeGaussianRasterization(gaussianFields);
        });
    }
    
    // Этап 3: Ray Traced Effects
    if (renderSettings.enableRayTracedGI && this->isRenderPassEnabled(RenderPass::RAY_TRACED_GI)) {
        hybridStats.rayTracingTime += this->measurePassTime([&]() {
            this->executeRayTracedGI();
        });
    }
    
    if (renderSettings.enableRayTracedReflections && this->isRenderPassEnabled(RenderPass::RAY_TRACED_REFLECTIONS)) {
        hybridStats.rayTracingTime += this->measurePassTime([&]() {
            this->executeRayTracedReflections();
        });
    }
    
    if (renderSettings.enableRayTracedShadows && this->isRenderPassEnabled(RenderPass::RAY_TRACED_SHADOWS)) {
        hybridStats.rayTracingTime += this->measurePassTime([&]() {
            this->executeRayTracedShadows();
        });
    }
    
    // Этап 4: Compositing
    if (this->isRenderPassEnabled(RenderPass::COMPOSITING)) {
        hybridStats.compositingTime += this->measurePassTime([&]() {
            this->executeCompositing();
        });
    }
    
    // Этап 5: AI Denoising
    if (renderSettings.enableDenoising) {
        hybridStats.denoisingTime += this->measurePassTime([&]() {
    ...
    if (renderSettings.enableNeuralUpscaling && renderSettings.upscalingFactor > 1.0f) {
        hybridStats.upscalingTime += this->measurePassTime([&]() {
            this->executeNeuralUpscaling();
        });
    }
}
...
void HybridRenderer3D::executeGaussianRasterization(const std::vector<std::shared_ptr<GaussianField3D>>& gaussianFields) {
    std::cout << "Выполнение растеризации гауссианов для " << gaussianFields.size() << " полей..." << std::endl;
...
void HybridRenderer3D::executeRayTracedGI() {
    ...
    int samplesPerPixel = static_cast<int>(renderSettings.rayTracingSamples * qualityAdjustmentFactor);
...
void HybridRenderer3D::executeRayTracedReflections() {
    ...
    int reflectionSamples = static_cast<int>(renderSettings.rayTracingSamples * 0.5f * qualityAdjustmentFactor);
...
void HybridRenderer3D::executeRayTracedShadows() {
    ...
    int shadowSamples = static_cast<int>(renderSettings.rayTracingSamples * 0.25f * qualityAdjustmentFactor);
...
void HybridRenderer3D::adjustRenderSettings() {
    ...
    renderSettings.rayTracingSamples = std::max(1, static_cast<int>(baseRaySamples * qualityAdjustmentFactor));
...
void ReSTIRGlobalIllumination::temporalResampling() {
    if (!settings.enableTemporalReuse) return;
...
void ReSTIRGlobalIllumination::spatialResampling() {
    if (!settings.enableSpatialReuse) return;
...
void AIDenoiser::denoise(unsigned int noisyBuffer, unsigned int outputBuffer,
                        unsigned int normalBuffer, unsigned int motionBuffer, 
                        unsigned int albedoBuffer) {
    if (!initialized) return;
    
    auto startTime = std::chrono::high_resolution_clock::now();
    
    std::cout << "Применение AI деноизинга (тип: " << static_cast<int>(settings.type) << ")..." << std::endl;
...
void NeuralUpscaler::upscale(unsigned int inputBuffer, unsigned int outputBuffer,
    ...
    std::cout << "Масштабирование " << inputWidth << "x" << inputHeight 
              << " -> " << outputWidth << "x" << outputHeight
              << " (тип: " << static_cast<int>(settings.type) << ")" << std::endl;
    
    switch (settings.type) {
        case UpscalerType::BILINEAR:
            applyBilinearUpscaling(inputBuffer, outputBuffer, inputWidth, inputHeight, outputWidth, outputHeight);
            break;
        case UpscalerType::LANCZOS:
            applyLanczosUpscaling(inputBuffer, outputBuffer, inputWidth, inputHeight, outputWidth, outputHeight);
            break;
        case UpscalerType::EDGE_ENHANCED:
            applyEdgeEnhancedUpscaling(inputBuffer, outputBuffer, inputWidth, inputHeight, outputWidth, outputHeight);
            break;
        default:
            break;
    }
