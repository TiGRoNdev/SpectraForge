...
void OptimalRenderer3D::HardwareConfig::autoDetect() {
    ...
    std::cout << "Определена конфигурация: "
              << "RT=" << (supportsRayTracing ? "Y" : "N")
              << ", Neural=" << (supportsNeural ? "Y" : "N")
              << ", MeshShaders=" << (supportsMeshShaders ? "Y" : "N")
              << ", VRS=" << (supportsVRS ? "Y" : "N")
              << ", Memory=" << gpuMemoryMB << "MB"
...
void OptimalRenderer3D::SceneData::calculateBounds() {
    ...
    for (const auto& field : gaussianFields) {
        if (field) {
            for (const auto& gaussian : field->getGaussians()) {
                ...
                minBounds = Vector3(
                    std::min(minBounds.x, gaussianMin.x),
                    std::min(minBounds.y, gaussianMin.y),
                    std::min(minBounds.z, gaussianMin.z)
                );
                maxBounds = Vector3(
                    std::max(maxBounds.x, gaussianMax.x),
    ...
    for (size_t i = 0; i < meshes.size() && i < transforms.size(); ++i) {
        if (meshes[i]) {
            ...
            minBounds = Vector3(
                std::min(minBounds.x, meshMin.x),
                std::min(minBounds.y, meshMin.y),
                std::min(minBounds.z, meshMin.z)
            );
            maxBounds = Vector3(
                std::max(maxBounds.x, meshMax.x),
    ...
    std::cout << "Границы сцены: центр(" << sceneCenter.x << ", " << sceneCenter.y << ", " << sceneCenter.z
              << "), радиус=" << sceneRadius << std::endl;
}
...
void OptimalRenderer3D::SceneData::optimizeForHardware(const HardwareConfig& config) {
    ...
    for (const auto& field : gaussianFields) {
        if (field) totalGaussians += field->getGaussianCount();
    }
    
    size_t totalTriangles = 0;
    for (const auto& mesh : meshes) {
        if (mesh) totalTriangles += mesh->getTriangleCount();
    }
    ...
    std::cout << "Уровень сложности сцены: " << complexityLevel 
              << " (" << totalGaussians << " гауссианов, " << totalTriangles << " треугольников)" << std::endl;
    
    // Адаптируем количество точек обзора для оптимизации
    int viewPointCount = std::max(1, std::min(8, static_cast<int>(config.performanceRating * complexityLevel)));
    ...
    if (multiViewImages.size() != static_cast<size_t>(viewPointCount)) {
        ...
        for (int i = 0; i < viewPointCount; ++i) {
            ...
            Vector3 viewPoint(
                sceneCenter.x + std::cos(angle) * distance,
    ...
    std::cout << "Создано " << multiViewImages.size() << " точек обзора для оптимизации" << std::endl;
}
...
bool OptimalRenderer3D::initialize(int width, int height, const HardwareConfig& hwConfig) {
    ...
    std::cout << "Инициализация оптимального рендерера (" << width << "x" << height << ")..." << std::endl;
    ...
    std::cout << "Оптимальный рендерер успешно инициализирован:"
              << "\n  Разрешение рендеринга: " << renderWidth << "x" << renderHeight
              << "\n  Целевое разрешение: " << targetWidth << "x" << targetHeight
...
void OptimalRenderer3D::renderOptimal3D(const SceneData& sceneData, 
    ...
    if (hardwareConfig.supportsNeural) {
        metrics.sceneOptimizationTime += measureExecutionTime([&]() {
            integrateNeRFVariants(workingSceneData);
        });
    }
    ...
    metrics.rayTracingTime = measureExecutionTime([&]() {
        executeAdvancedLighting(workingSceneData);
    });
    
    // Этап 4: Denoising and Refinement
    metrics.denoisingTime = measureExecutionTime([&]() {
        executeDenoisingAndRefinement();
    });
    
    // Этап 5: Post-Processing and Output
    metrics.upscalingTime = measureExecutionTime([&]() {
        executePostProcessingAndOutput(targetWidth, targetHeight);
    });
...
void OptimalRenderer3D::optimizeSceneRepresentation(SceneData& sceneData, const HardwareConfig& hwConfig) {
    ...
    if (cacheValid) {
        for (size_t i = 0; i < sceneData.multiViewImages.size(); ++i) {
            float distance = (sceneData.multiViewImages[i] - optimizationCache.lastViewPoints[i]).magnitude();
...
void OptimalRenderer3D::executeGeometryAndVisibility(const SceneData& sceneData, const CameraParams& cameraParams) {
    ...
    camera->setPerspective(cameraParams.fov, cameraParams.aspectRatio, 
...
void OptimalRenderer3D::executeAdvancedLighting(const SceneData& sceneData) {
    ...
    if (globalIllumination && globalIlluminationEnabled) {
        ...
        Matrix4 viewMatrix = Matrix4::lookAt(lastCameraParams.position, lastCameraParams.target, lastCameraParams.up);
        Matrix4 projMatrix = Matrix4::perspective(lastCameraParams.fov, lastCameraParams.aspectRatio, 
                                                  lastCameraParams.nearPlane, lastCameraParams.farPlane);
        
        // TODO: Передача правильных буферов
        globalIllumination->computeGlobalIllumination(
            viewMatrix,
            projMatrix,
            0, 0, 0 // TODO: Правильные буферы
        );
    ...
    std::cout << "Трассировка лучей завершена (время: " << metrics.rayTracingTime << "мс)" << std::endl;
}
...
void OptimalRenderer3D::adaptToHardware() {
    ...
    std::cout << "Адаптация под железо: разрешение рендеринга " << renderWidth << "x" << renderHeight
              << " (масштаб производительности: " << performanceScale << ")" << std::endl;
}
...
void OptimalRenderer3D::optimizeForFrameRate() {
    ...
    if (frameTimeHistory.size() < 5) return; // Недостаточно данных
...
void OptimalRenderer3D::balanceQualityAndPerformance() {
    ...
    std::cout << "Анализ производительности:"
              << " растеризация=" << (rasterizationRatio * 100) << "%"
    ...
    if (hybridRenderer) {
        ...
        if (rayTracingRatio > 0.4f) {
            ...
            std::cout << "Снижение количества ray-tracing samples до " << settings.rayTracingSamples << std::endl;
        }
        
        if (rasterizationRatio > 0.5f) {
            // Растеризация - узкое место
            settings.gaussianQuality *= 0.9f;
            std::cout << "Снижение качества гауссианов до " << settings.gaussianQuality << std::endl;
        }
...
void OptimalRenderer3D::updateMetrics() {
    ...
    float baselineTime = (metrics.gaussiansProcessed * 0.001f) + (metrics.trianglesRendered * 0.0001f);
    metrics.renderingSpeedup = baselineTime > 0 ? baselineTime / metrics.totalFrameTime : 1.0f;
    
    // Эффективность памяти (упрощенная оценка)
    float memoryUsed = (metrics.gaussiansProcessed * 64.0f + metrics.trianglesRendered * 48.0f) / (1024.0f * 1024.0f);
    metrics.memoryEfficiency = currentHardware.gpuMemoryMB > 0 ? 
                              (1.0f - memoryUsed / currentHardware.gpuMemoryMB) : 0.5f;
    
    // Качество визуализации (на основе включенных эффектов)
    float qualityScore = 0.5f; // Базовое качество
    if (globalIlluminationEnabled) qualityScore += 0.2f;
    if (reflectionsEnabled) qualityScore += 0.15f;
    if (shadowsEnabled) qualityScore += 0.1f;
    if (denoisingEnabled) qualityScore += 0.05f;
...
void OptimalRenderer3D::runBenchmark(const SceneData& testScene, int frames) {
    ...
    for (int frame = 0; frame < frames; ++frame) {
        ...
        benchmarkCamera.position = Vector3(
            testScene.sceneCenter.x + std::cos(angle) * testScene.sceneRadius * 2,
        ...
        if (frame % 10 == 0) {
            std::cout << "Кадр " << frame << "/" << frames 
                      << " (время: " << metrics.totalFrameTime << "мс)" << std::endl;
        }
    }
...
float OptimalRenderer3D::PerformanceMetrics::getTotalTime() const {
    return sceneOptimizationTime + rasterizationTime + rayTracingTime + 
           denoisingTime + upscalingTime + postProcessingTime;
}
...
std::unique_ptr<OptimalRenderer3D> OptimalRendererFactory::createOptimalRenderer(const CreationParams& params) {
...
OptimalRendererFactory::CreationParams OptimalRendererFactory::recommendSettings(const OptimalRenderer3D::HardwareConfig& hw) {
...
void AdaptiveLOD::computeLOD(GaussianField3D& field, 
    ...
    std::cout << "Вычисление адаптивного LOD для " << field.getGaussianCount() << " гауссианов..." << std::endl;
    ...
    std::cout << "LOD распределение: "
              << "L0=" << stats.gaussiansPerLevel[0] 
              << ", L1=" << stats.gaussiansPerLevel[1]
              << ", L2=" << stats.gaussiansPerLevel[2] 
...
void AdaptiveLOD::adaptToPerformance(float currentFrameTime, float targetFrameTime) {
    ...
    if (frameTimeHistory.size() < 3) return;
...
int AdaptiveLOD::computeLODLevel(float distance, float screenSize, float velocity) const {
    ...
    int baseLOD = 0;
    if (distance > settings.lowDetailDistance) baseLOD = 3;
    else if (distance > settings.mediumDetailDistance) baseLOD = 2;
    else if (distance > settings.highDetailDistance) baseLOD = 1;
    
    // Корректировка на основе размера на экране
    if (screenSize < 0.01f) baseLOD = std::min(3, baseLOD + 1);
    else if (screenSize > 0.1f) baseLOD = std::max(0, baseLOD - 1);
    
    // Корректировка на основе скорости движения
    if (velocity > 5.0f) baseLOD = std::min(3, baseLOD + 1);
...
void AdaptiveLOD::applyMotionBasedLOD(GaussianField3D& field, const Vector3& velocity) {
    if (velocity.magnitude() < 1.0f) return; // Статическая камера
    
    std::cout << "Применение LOD на основе движения (скорость: " << velocity.magnitude() << ")..." << std::endl;
