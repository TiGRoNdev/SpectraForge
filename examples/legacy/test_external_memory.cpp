...
int main() {
    try {
        ...
        if (!cudaInterop.initializeInterop(
                engine.getDevice(), engine.getPhysicalDevice(), engine.getResourceManager())) {
        ...
        if (sharedBuffer && sharedBuffer->isValid) {
            ...
            std::cout << "  Vulkan буфер: 0x" << std::hex
                      << reinterpret_cast<uintptr_t>(
                             static_cast<VkBuffer>(sharedBuffer->vulkanBuffer))
                      << std::dec << std::endl;
            std::cout << "  CUDA pointer: 0x" << std::hex << sharedBuffer->cudaDevicePtr << std::dec
                      << std::endl;
        ...
        if (syncObject && syncObject->isValid) {
            ...
            std::cout << "  Vulkan semaphore: 0x" << std::hex
                      << reinterpret_cast<uintptr_t>(
                             static_cast<VkSemaphore>(syncObject->vulkanSemaphore))
                      << std::dec << std::endl;
        } else {
