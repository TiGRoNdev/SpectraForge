    ...
    for (auto backend : availableBackends) {
        ...
        if (adapter.setBackend(backend)) {
            ...
            if (adapter.initialize(640, 480)) {
                ...
                std::vector<std::string> features = {"basic_rendering",
                                                     "wireframe",
                                                     "depth_test",
                                                     "blending",
                                                     "ray_tracing",
...
void demonstrateErrorHandling() {
    ...
    std::vector<RenderBackend> allBackends = {RenderBackend::OPENGL, RenderBackend::VULKAN};
...
void performanceComparison() {
    ...
    for (auto backend : availableBackends) {
        ...
        std::cout << "  - Время на кадр: " << (duration.count() / (float)TEST_FRAMES) << " мс"
                  << std::endl;
