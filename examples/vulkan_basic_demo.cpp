...
int main() {
    try {
        ...
        if (hardwareDetector) {
            ...
            if (vramSize > 0) {
                std::cout << "   VRAM: " << SAFE_TO_STRING(vramSize / (1024 * 1024)) << " MB"
                          << std::endl;
            } else {
            ...
            std::cout << "   Ray Tracing: "
                      << (hardwareDetector->supportsRayTracing() ? "Поддерживается"
                                                                 : "Не поддерживается")
                      << std::endl;
            std::cout << "   CUDA: "
                      << (hardwareDetector->supportsCUDA() ? "Поддерживается" : "Не поддерживается")
                      << std::endl;
            std::cout << "   OptiX: "
                      << (hardwareDetector->supportsOptiX() ? "Поддерживается"
                                                            : "Не поддерживается")
                      << std::endl;
        ...
        if (sceneManager) {
            try {
                ...
                if (sceneManager->loadScene(testScene)) {
                    ...
                    std::cout << "   Объектов в сцене: " << sceneManager->getObjectCount()
                              << std::endl;
                    ...
                    std::cout << "   Гауссианов для рендеринга: " << SAFE_TO_STRING(gaussians.count)
                              << std::endl;
                } else {
