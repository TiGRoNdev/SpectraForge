...
bool AccelerationStructure::build(const SceneGeometry& geometry) {
    ...
    std::cout << "  Вершин: " << geometry.vertexCount
              << ", Треугольников: " << geometry.triangleCount << std::endl;
...
void OptiXRayTracer::buildAccelerationStructures(const SceneGeometry& geo) {
    ...
    std::cout << "  Вершин: " << geo.vertexCount << ", Треугольников: " << geo.triangleCount
              << std::endl;
...
RawEffects OptiXRayTracer::traceRays(const LaunchParams& params) {
    ...
    std::cout << "[OptiXRayTracer] Трассировка лучей " << params.width << "x" << params.height
              << std::endl;
...
void OptiXRayTracer::freeBuffers() {
    ...
    if (d_reflections) {
        cudaFree(reinterpret_cast<void*>(d_reflections));
        d_reflections = 0;
    }
    if (d_shadows) {
        cudaFree(reinterpret_cast<void*>(d_shadows));
        d_shadows = 0;
    }
    if (d_globalIllumination) {
        cudaFree(reinterpret_cast<void*>(d_globalIllumination));
        d_globalIllumination = 0;
    }
    if (d_motionVectors) {
        cudaFree(reinterpret_cast<void*>(d_motionVectors));
        d_motionVectors = 0;
    }
    if (d_albedo) {
        cudaFree(reinterpret_cast<void*>(d_albedo));
        d_albedo = 0;
    }
    if (d_normals) {
...
void OptiXRayTracer::logCallback(unsigned int level,
                                 const char* tag,
                                 const char* message,
                                 void* cbdata) {
