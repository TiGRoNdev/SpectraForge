...
bool CudaInterop::initializeInterop(vk::Device dev,
    ...
    try {
        ...
        if (!checkExternalSemaphoreSupport()) {
            std::cout << "[CudaInterop] Предупреждение: External semaphores не поддерживаются"
                      << std::endl;
        }

        // Ищем совпадающий CUDA device
        if (!findMatchingCudaDevice()) {
            std::cout << "[CudaInterop] Ошибка: Не найдено совпадающее CUDA устройство"
                      << std::endl;
            return false;
        }
...
std::shared_ptr<SharedResource> CudaInterop::createSharedBuffer(size_t size,
    ...
    try {
        ...
        SAFE_PRINT_LINE("[CudaInterop] Создание shared буфера размером " + SAFE_TO_STRING(size)
                        + " байт");
        ...
        cudaError_t result =
            cudaExternalMemoryGetMappedBuffer(reinterpret_cast<void**>(&resource->cudaDevicePtr),
...
std::shared_ptr<SyncObject> CudaInterop::createSyncObject() {
    ...
    try {
        ...
        try {
            std::cout << "[CudaInterop] Временная заглушка для semaphore handle (требуется "
            ...
            cudaError_t result =
                cudaImportExternalSemaphore(&syncObj->cudaExternalSemaphore, &semHandleDesc);
        ...
        std::cout << "[CudaInterop] Ошибка создания объекта синхронизации: " << e.what()
                  << std::endl;
        return nullptr;
    }
}
...
void CudaInterop::signalVulkanToCuda(const std::shared_ptr<SyncObject>& syncObj, cudaStream_t stream) {
    ...
    cudaError_t result =
        cudaWaitExternalSemaphoresAsync(&syncObj->cudaExternalSemaphore, &waitParams, 1, stream);
...
cudaExternalMemory_t CudaInterop::importVulkanMemory(vk::DeviceMemory vulkanMemory, size_t size) {
    try {
        ...
        try {
            std::cout << "[CudaInterop] Временная заглушка для memory handle (требуется "
...
bool CudaInterop::isInteropSupported() {
    ...
    try {
        ...
        if (result != cudaSuccess) {
            std::cout << "[CudaInterop] CUDA runtime недоступен: " << cudaGetErrorString(result)
                      << std::endl;
            return false;
        }
        ...
        std::cout << "[CudaInterop] Найдено CUDA устройств: " << SAFE_TO_STRING(deviceCount)
                  << std::endl;
        ...
        for (int device = 0; device < deviceCount; ++device) {
            ...
            std::cout << "[CudaInterop] Устройство " << SAFE_TO_STRING(device) << ": " << prop.name
                      << std::endl;
            std::cout << "[CudaInterop] Compute Capability: " << SAFE_TO_STRING(prop.major) << "."
                      << SAFE_TO_STRING(prop.minor) << std::endl;
        ...
        std::cout << "[CudaInterop] Ни одно устройство не поддерживает external memory (требуется "
                     "Compute >= 6.0)"
                  << std::endl;
        return false;
...
bool CudaInterop::initCudaContext() {
    try {
        ...
        if (result != cudaSuccess) {
            std::cout << "[CudaInterop] Ошибка установки CUDA устройства "
                      << SAFE_TO_STRING(cudaDevice) << ": " << cudaGetErrorString(result)
                      << std::endl;
            return false;
        }
        ...
        if (cudaContext == nullptr) {
            ...
            if (cuResult != CUDA_SUCCESS || cudaContext == nullptr) {
                std::cout << "[CudaInterop] Ошибка получения созданного CUDA контекста"
                          << std::endl;
                return false;
            }
        }
...
bool CudaInterop::checkExternalMemorySupport() {
    ...
    try {
        ...
        if (result != cudaSuccess) {
            std::cout << "[CudaInterop] Ошибка получения свойств устройства: "
                      << cudaGetErrorString(result) << std::endl;
            return false;
        }
        ...
        std::cout << "[CudaInterop] Устройство " << SAFE_TO_STRING(cudaDevice) << " (" << prop.name
                  << ")" << std::endl;
        std::cout << "[CudaInterop] Compute Capability: " << SAFE_TO_STRING(prop.major) << "."
                  << SAFE_TO_STRING(prop.minor) << std::endl;
        std::cout << "[CudaInterop] External Memory поддержка: "
...
bool CudaInterop::checkExternalSemaphoreSupport() {
    if (cudaDevice < 0) {
        std::cout << "[CudaInterop] CUDA устройство не инициализировано для проверки semaphore"
                  << std::endl;
        return false;
    }
    ...
    try {
        ...
        if (result != cudaSuccess) {
            std::cout << "[CudaInterop] Ошибка получения свойств для semaphore: "
                      << cudaGetErrorString(result) << std::endl;
            return false;
        }
        ...
        std::cout << "[CudaInterop] External Semaphore поддержка: "
    ...
    try {
        ...
        for (const auto& required : requiredExtensions) {
            ...
            if (found) {
                ...
                std::cout << "[CudaInterop] ✗ Расширение НЕ поддерживается: " << required
                          << std::endl;
                return false;
            }
        }
...
bool CudaInterop::findMatchingCudaDevice() {
    ...
    if (cudaDevice >= 0) {
        std::cout << "[CudaInterop] Используем CUDA устройство " << SAFE_TO_STRING(cudaDevice)
                  << std::endl;
        return true;
    }
...
void cuda_interop_test() {
    ...
    if (HyperEngine::CUDA::CudaInterop::isInteropSupported()) {
        ...
        std::cout << "[CudaInterop Test] Interop не поддерживается на данной платформе"
                  << std::endl;
    }
}
}
