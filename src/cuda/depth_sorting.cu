 ...
 */
__global__ void prepareSortPairs(const HyperEngine::CUDA::ProjectedGaussian* projected,
 ...
 * @param numGaussians Количество гауссианов
 */
__global__ void extractSortedIndices(const DepthSortPair* sortPairs,
 ...
 */
__global__ void localBitonicSort(DepthSortPair* sortPairs, int groupSize, int numGroups) {
 ...
 */
__global__ void sortByTiles(const HyperEngine::CUDA::ProjectedGaussian* projected,
 ...
 * @param numTiles Количество тайлов
 */
__global__ void buildTileLists(const uint32_t* tileAssignments,
 ...
 */
__global__ void extractKeysValuesKernel(const DepthSortPair* pairs,
                                        float* keys,
                                        uint32_t* values,
                                        int n) {
    int idx = blockIdx.x * blockDim.x + threadIdx.x;
...
cudaError_t launchCubRadixSort(const HyperEngine::CUDA::ProjectedGaussian* d_projected,
    ...
    try {
        ...
        cub::DeviceRadixSort::SortPairs(d_tempStorage,
                                        tempStorageBytes,
                                        (float*)nullptr,
                                        (float*)nullptr,  // keys
                                        (uint32_t*)nullptr,
                                        (uint32_t*)nullptr,  // values
                                        numGaussians,
                                        0,
                                        sizeof(float) * 8,
                                        stream);
        ...
        cudaMemcpy(d_sortedIndices,
                   d_valuesOut,
                   numGaussians * sizeof(uint32_t),
 ...
 */
cudaError_t launchThrustSort(const HyperEngine::CUDA::ProjectedGaussian* d_projected,
                             uint32_t* d_sortedIndices,
                             int numGaussians,
                             cudaStream_t stream = 0) {
    using namespace HyperEngine::CUDA;
    ...
    try {
        ...
        thrust::sort(
            thrust::cuda::par.on(stream), sortPairs.begin(), sortPairs.end(), DepthComparator());
...
cudaError_t launchTileAwareSorting(const HyperEngine::CUDA::ProjectedGaussian* d_projected,
    ...
    try {
        ...
        buildTileLists<<<gridSize, blockSize, 0, stream>>>(
            d_tileAssignments, d_sortedIndices, d_tileOffsets, numGaussians, numTiles);
        ...
        sortByTiles<<<tileGridSize, tileBlockSize, sharedMemSize, stream>>>(
            d_projected, d_tileAssignments, d_sortedIndices, d_tileOffsets, numGaussians, numTiles);
 ...
 */
cudaError_t launchBitonicSort(const HyperEngine::CUDA::ProjectedGaussian* d_projected,
                              uint32_t* d_sortedIndices,
                              int numGaussians,
                              cudaStream_t stream = 0) {
    using namespace HyperEngine::CUDA;
