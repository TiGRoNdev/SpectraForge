 ...
 */
__global__ void initializeGaussiansFromPoints(const float4* points,
 ...
 */
__global__ void computePositionGradients(const GPUGaussian* gaussians,
    ...
    for (int dy = -radius; dy <= radius; dy++) {
        for (int dx = -radius; dx <= radius; dx++) {
            ...
            if (x >= 0 && x < width && y >= 0 && y < height) {
                ...
                float4 diff = make_float4(imageTarget[pixelIdx].x - imageRendered[pixelIdx].x,
 ...
 * @param numGaussians Количество гауссианов
 */
__global__ void updateGaussianParameters(GPUGaussian* gaussians,
    ...
    float gradientMagnitude =
        sqrtf(gradient.x * gradient.x + gradient.y * gradient.y + gradient.z * gradient.z);
 ...
 */
__global__ void densificationSplit(const GPUGaussian* gaussians,
                                   GPUGaussian* newGaussians,
                                   const bool* splitMask,
                                   int numGaussians,
                                   int* numNewGaussians) {
    int idx = blockIdx.x * blockDim.x + threadIdx.x;
    if (idx >= numGaussians || !splitMask[idx])
        return;
 ...
 */
__global__ void pruneWeakGaussians(const GPUGaussian* gaussians,
                                   bool* validMask,
                                   int numGaussians,
                                   float minOpacity) {
    int idx = blockIdx.x * blockDim.x + threadIdx.x;
...
cudaError_t launchGradientComputation(const HyperEngine::CUDA::GPUGaussian* d_gaussians,
    ...
    computePositionGradients<<<gridSize, blockSize, 0, stream>>>(
        d_gaussians, d_imageTarget, d_imageRendered, d_gradients, width, height, numGaussians);
