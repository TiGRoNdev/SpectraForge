...
Collider3D::Collider3D(Type type)
    : type(type), center(0, 0, 0), transform(Matrix4::identity()), trigger(false), enabled(true) {}

// SphereCollider3D implementation
SphereCollider3D::SphereCollider3D(float radius) : Collider3D(Type::Sphere), radius(radius) {}
...
RaycastHit3D SphereCollider3D::raycast(const Vector3& origin,
...
BoxCollider3D::BoxCollider3D(const Vector3& size) : Collider3D(Type::Box), size(size) {}
...
Vector3 BoxCollider3D::getNormal(const Vector3& point) const {
    ...
    Vector3 absLocal =
        Vector3(std::abs(localPoint.x), std::abs(localPoint.y), std::abs(localPoint.z));
    Vector3 relativePos =
        Vector3(absLocal.x / halfSize.x, absLocal.y / halfSize.y, absLocal.z / halfSize.z);
    ...
    if (other.getType() == Type::Box) {
        ...
        if (minA.x <= maxB.x && maxA.x >= minB.x && minA.y <= maxB.y && maxA.y >= minB.y
            ...
            Vector3 intersectionMin = Vector3(
                std::max(minA.x, minB.x), std::max(minA.y, minB.y), std::max(minA.z, minB.z));
            Vector3 intersectionMax = Vector3(
                std::min(maxA.x, maxB.x), std::min(maxA.y, maxB.y), std::min(maxA.z, maxB.z));
...
RaycastHit3D BoxCollider3D::raycast(const Vector3& origin,
                                    const Vector3& direction,
                                    float maxDistance) const {
    RaycastHit3D hit;
...
bool PlaneCollider3D::intersects(const Collider3D& other) const {
    ...
    switch (other.getType()) {
        ...
        case Type::Box: {
            ...
            std::vector<Vector3> vertices = {Vector3(min.x, min.y, min.z),
...
RaycastHit3D PlaneCollider3D::raycast(const Vector3& origin,
                                      const Vector3& direction,
                                      float maxDistance) const {
    RaycastHit3D hit;
...
void RigidBody3D::applyAngularImpulse(const Vector3& impulse) {
    if (!kinematic) {
        ...
        angularVelocity +=
            Vector3(impulse.x * invInertia.x, impulse.y * invInertia.y, impulse.z * invInertia.z);
    }
}
...
Vector3 RigidBody3D::getInverseInertia() const {
    return Vector3(inertia.x > 0.0f ? 1.0f / inertia.x : 0.0f,
    ...
    for (int i = 0; i < count; ++i) {
        ...
        if (particle) {
            ...
            particle->velocity = emitterVelocity
                                 + Vector3((rand() / float(RAND_MAX) - 0.5f) * 2.0f,
...
PhysicsWorld3D::PhysicsWorld3D() : gravity(0, -9.81f, 0), timeStep(1.0f / 60.0f), iterations(4) {}
    ...
    for (size_t i = 0; i < rigidBodies.size(); ++i) {
        for (size_t j = i + 1; j < rigidBodies.size(); ++j) {
            ...
            if (bodyA->getCollider() && bodyB->getCollider()) {
                CollisionResult3D result =
                    bodyA->getCollider()->checkCollision(*bodyB->getCollider());
...
RaycastHit3D PhysicsWorld3D::raycast(const Vector3& origin,
                                     const Vector3& direction,
                                     float maxDistance) {
...
std::vector<RaycastHit3D> PhysicsWorld3D::raycastAll(const Vector3& origin,
                                                     const Vector3& direction,
                                                     float maxDistance) {
