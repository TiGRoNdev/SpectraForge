...
bool Renderer3D::initialize(int width, int height) {
    ...
    SAFE_PRINT_LINE("Initializing Renderer3D with resolution " + SAFE_TO_STRING(width) + "x"
...
void Renderer3D::clear() {
    ...
    SAFE_PRINT_LINE("Clearing buffers with color (" + SAFE_TO_STRING(clearColor.x) + ", "
                    + SAFE_TO_STRING(clearColor.y) + ", " + SAFE_TO_STRING(clearColor.z) + ")");
}

// Настройки рендеринга
void Renderer3D::setClearColor(float r, float g, float b, float a) {
    clearColor = Vector3(r, g, b);
    SAFE_PRINT_LINE("Set clear color to (" + SAFE_TO_STRING(r) + ", " + SAFE_TO_STRING(g) + ", "
                    + SAFE_TO_STRING(b) + ", " + SAFE_TO_STRING(a) + ")");
}

void Renderer3D::setViewport(int x, int y, int width, int height) {
    // TODO: Установка viewport для конкретного API
    SAFE_PRINT_LINE("Set viewport to (" + SAFE_TO_STRING(x) + ", " + SAFE_TO_STRING(y) + ", "
                    + SAFE_TO_STRING(width) + ", " + SAFE_TO_STRING(height) + ")");
}
...
// Рендеринг объектов
void Renderer3D::renderMesh(const Mesh3D& mesh,
...
void Renderer3D::renderMesh(std::shared_ptr<Mesh3D> mesh,
                            const Math::Matrix4& transform,
...
void Renderer3D::renderWireframe(const Mesh3D& mesh,
                                 const Math::Matrix4& transform,
                                 const Shader3D& shader) {
