...
std::shared_ptr<Mesh3D> Mesh3D::createSphere(float radius, int segments) {
    ...
    for (int lat = 0; lat <= segments; ++lat) {
        ...
        for (int lon = 0; lon <= segments; ++lon) {
            ...
            Vector3 position(radius * cosPhi * sinTheta, radius * cosTheta, radius * sinPhi * sinTheta);
...
std::shared_ptr<Mesh3D> Mesh3D::createCylinder(float radius, float height, int segments) {
    ...
    // Центры оснований
    cylinderVertices.emplace_back(Vector3(0, halfHeight, 0), Vector3(0, 1, 0), Vector3(1, 1, 1), 0.5f, 0.5f);   // Верх
    cylinderVertices.emplace_back(Vector3(0, -halfHeight, 0), Vector3(0, -1, 0), Vector3(1, 1, 1), 0.5f, 0.5f); // Низ
    ...
    for (int i = 0; i <= segments; ++i) {
        ...
        // Верхняя окружность
        cylinderVertices.emplace_back(Vector3(x, halfHeight, z), Vector3(0, 1, 0), Vector3(1, 1, 1), u, 0);
        // Нижняя окружность
        cylinderVertices.emplace_back(Vector3(x, -halfHeight, z), Vector3(0, -1, 0), Vector3(1, 1, 1), u, 1);
    }
