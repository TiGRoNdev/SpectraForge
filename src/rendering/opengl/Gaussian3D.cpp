...
void Gaussian3D::updateCovariance() {
    ...
    Matrix4 rotationMatrix = Matrix4::rotationX(rotation.x) * Matrix4::rotationY(rotation.y)
...
bool Gaussian3D::intersects(const Vector3& min, const Vector3& max) const {
    ...
    return !(gaussianMax.x < min.x || gaussianMin.x > max.x || gaussianMax.y < min.y
             || gaussianMin.y > max.y || gaussianMax.z < min.z || gaussianMin.z > max.z);
}
...
void GaussianField3D::optimizeGaussians(const std::vector<Vector3>& viewPoints) {
    ...
    std::sort(gaussians.begin(), gaussians.end(), [](const Gaussian3D& a, const Gaussian3D& b) {
...
void GaussianField3D::sortByDistance(const Vector3& viewPoint) {
    ...
    std::sort(visibleIndices.begin(), visibleIndices.end(), [this, &viewPoint](size_t a, size_t b) {
...
std::shared_ptr<GaussianField3D> GaussianField3D::createFromMesh(const Mesh3D& mesh,
                                                                 int gaussianDensity) {
    ...
    std::cout << "Создание поля гауссианов из меша с " << vertices.size() << " вершинами и "
              << indices.size() / 3 << " треугольниками..." << std::endl;
    ...
    if (gaussianDensity > static_cast<int>(vertices.size())) {
        ...
        for (int i = 0; i < additionalGaussians && i < triangleCount * 3; ++i) {
            ...
            gaussian.scale =
                Vector3(gaussianScale * 0.7f, gaussianScale * 0.7f, gaussianScale * 0.7f);
...
std::shared_ptr<GaussianField3D> GaussianField3D::createFromPointCloud(
    ...
    std::cout << "Создание поля гауссианов из облака точек (" << points.size() << " точек)..."
              << std::endl;
    ...
    std::cout << "Создано " << field->getGaussianCount() << " гауссианов из облака точек"
              << std::endl;
    return field;
}
...
void GaussianField3D::setLevelOfDetail(float distance, int maxGaussians) {
    ...
    if (static_cast<int>(gaussians.size()) > targetGaussians) {
        std::partial_sort(
            gaussians.begin(),
            gaussians.begin() + targetGaussians,
            gaussians.end(),
            [](const Gaussian3D& a, const Gaussian3D& b) { return a.importance > b.importance; });
...
void GaussianField3D::buildSpatialTree() {
    ...
    std::cout << "Построение пространственного дерева для " << gaussians.size() << " гауссианов..."
              << std::endl;
}
...
void GaussianRenderer3D::render(const GaussianField3D& field,
    ...
    std::cout << "Рендеринг сцены с " << field.getGaussianCount()
              << " гауссианами (разрешение: " << screenWidth << "x" << screenHeight << ")"
              << std::endl;
    ...
    std::cout << "Рендеринг завершен. Время: " << renderStats.rasterizationTime << "мс"
              << std::endl;
}
...
void GaussianRenderer3D::setSortingMethod(int method) {
    ...
    std::cout << "Метод сортировки установлен: " << (method == 0 ? "по глубине" : "по важности")
              << std::endl;
}
...
void GaussianRenderer3D::sortGaussians(std::vector<Gaussian3D>& gaussians,
                                       const Vector3& viewPoint) {
    if (sortingMethod == 0) {
        // Сортировка по глубине
        std::sort(gaussians.begin(),
                  gaussians.end(),
...
void GaussianRenderer3D::renderTiles(const GaussianField3D& field,
                                     int screenWidth,
                                     int screenHeight) {
    ...
    std::cout << "Тайловый рендеринг: " << tilesX << "x" << tilesY << " = " << totalTiles
              << " тайлов" << std::endl;
