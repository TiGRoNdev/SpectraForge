...
bool SceneManager::loadScene(const SceneData& data) {
    ...
    try {
        ...
        sceneLoaded = true;
        std::cout << "[SceneManager] Сцена загружена успешно. Объектов: " << objectCount
                  << std::endl;
        return true;
...
uint32_t SceneManager::addObject(const std::string& objectPath) {
    ...
    try {
        ...
        std::cout << "[SceneManager] Добавлен объект: " << objectPath << " (ID: " << obj.id << ")"
                  << std::endl;
...
void SceneManager::removeObject(uint32_t objectId) {
    ...
    auto it = std::find_if(sceneObjects.begin(),
                           sceneObjects.end(),
...
void SceneManager::generateGaussians() {
    std::cout << "[SceneManager] Генерация гауссианов из " << objectCount << " объектов..."
              << std::endl;
