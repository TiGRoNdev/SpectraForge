...
class CudaVulkanInteropTest : public HyperEngineTest {
  ...
  private:
    ...
    void setupVulkanContext() {
        ...
        std::vector<const char*> extensions = {VK_KHR_EXTERNAL_MEMORY_EXTENSION_NAME,
    ...
    EXPECT_NO_THROW_WITH_MESSAGE(
        {
            ...
            EXPECT_CALL(*mockFlashGS, initialize(testing::_)).WillOnce(testing::Return(true));

            bool flashGSResult = mockFlashGS->initialize(cudaContext);
            EXPECT_TRUE(flashGSResult);

            // Инициализация OptiX с CUDA контекстом
            EXPECT_CALL(*mockOptiX, init(testing::_)).WillOnce(testing::Return(true));
            ...
            bool resourceResult =
                mockResourceManager->initialize(vulkanDevice, vulkanPhysicalDevice);
    ...
    EXPECT_NO_THROW_WITH_MESSAGE(
        {
            ...
            vulkanBuffer =
                mockResourceManager->createBuffer(testBufferSize,
            ...
            // Размаппинг
            EXPECT_CALL(*mockResourceManager, unmapCudaMemory(testing::_)).Times(1);
    ...
    EXPECT_NO_THROW_WITH_MESSAGE(
        {
            ...
            EXPECT_CALL(*mockFlashGS, optimizeGaussians(testing::_)).Times(1);
    ...
    EXPECT_NO_THROW_WITH_MESSAGE(
        {
            ...
            EXPECT_CALL(*mockOptiX, buildAccelerationStructures(testing::_)).Times(1);
            ...
            EXPECT_CALL(*mockOptiX, applySER(testing::_)).Times(1);
    ...
    EXPECT_PERFORMANCE_UNDER(
        {
            for (int i = 0; i < operationCount; ++i) {
                ...
                EXPECT_CALL(*mockResourceManager, mapCudaMemory(testing::_))
                    .WillOnce(testing::Return(cudaPtr));
                EXPECT_CALL(*mockResourceManager, unmapCudaMemory(testing::_)).Times(1);
                EXPECT_CALL(*mockResourceManager, destroyBuffer(testing::_, testing::_)).Times(1);

                buffer =
                    mockResourceManager->createBuffer(1024,
                                                      vk::BufferUsageFlagBits::eStorageBuffer,
                                                      vk::MemoryPropertyFlagBits::eDeviceLocal);
    ...
    EXPECT_NO_THROW_WITH_MESSAGE(
        {
            ...
            EXPECT_CALL(*mockFlashGS, initialize(testing::_)).WillOnce(testing::Return(false));

            bool result = mockFlashGS->initialize(nullptr);
            EXPECT_FALSE(result);

            // Попытка работы с неинициализированным OptiX
            EXPECT_CALL(*mockOptiX, init(testing::_)).WillOnce(testing::Return(false));
