...
TEST_F(RenderingPipelineIntegrationTest, FullPipelineInitialization) {
    EXPECT_NO_THROW_WITH_MESSAGE({
        ...
        bool initResult = rendererAdapter->initialize(RenderBackend::AUTO, renderWidth, renderHeight);
...
TEST_F(RenderingPipelineIntegrationTest, HybridRenderingPipeline) {
    ...
    EXPECT_NO_THROW_WITH_MESSAGE({
        ...
        bool initResult = rendererAdapter->initialize(RenderBackend::VULKAN, renderWidth, renderHeight);
        EXPECT_TRUE(initResult);
...
TEST_F(RenderingPipelineIntegrationTest, BackendSwitching) {
    EXPECT_NO_THROW_WITH_MESSAGE({
        // Начинаем с OpenGL
        bool initResult = rendererAdapter->initialize(RenderBackend::OPENGL, renderWidth, renderHeight);
        EXPECT_TRUE(initResult);
...
TEST_F(RenderingPipelineIntegrationTest, ErrorRecovery) {
    EXPECT_NO_THROW_WITH_MESSAGE({
        ...
        bool goodInit = rendererAdapter->initialize(RenderBackend::OPENGL, renderWidth, renderHeight);
...
TEST_F(RenderingPipelineIntegrationTest, DifferentObjectTypes) {
    ...
    EXPECT_NO_THROW_WITH_MESSAGE({
        ...
        Engine3D::Math::Matrix4 wireTransform = Engine3D::Math::Matrix4::translation(2.0f, 0.0f, 0.0f);
        ...
        Engine3D::Math::Matrix4 blendTransform = Engine3D::Math::Matrix4::translation(-2.0f, 0.0f, 0.0f);
        ...
        for (int frame = 0; frame < longRunFrames; ++frame) {
            try {
                ...
                for (int obj = 0; obj < objectCount; ++obj) {
                    Engine3D::Math::Matrix4 transform = Engine3D::Math::Matrix4::translation(
                        std::sin(frame * 0.01f + obj), 
                        std::cos(frame * 0.01f + obj), 
                        0.0f
                    );
...
class RenderingPipelineConfigTest : public RenderingPipelineIntegrationTest,
...
TEST_P(RenderingPipelineConfigTest, DifferentConfigurations) {
    ...
    }, "Конфигурация " + std::to_string(static_cast<int>(backend)) + 
...
INSTANTIATE_TEST_SUITE_P(
    ...
    ::testing::Values(
        std::make_tuple(RenderBackend::OPENGL, 800, 600),
