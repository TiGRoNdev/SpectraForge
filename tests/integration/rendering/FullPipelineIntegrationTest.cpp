...
private:
    ...
    void setupTestScene() {
        ...
        SAFE_PRINT_LINE("[Pipeline] Сцена настроена: " 
                       + SAFE_TO_STRING(sceneGaussiansCount) + " гауссианов, " 
                       + SAFE_TO_STRING(sceneWidth) + "x" + SAFE_TO_STRING(sceneHeight));
    }
...
TEST_F(FullPipelineIntegrationTest, CompletePipelineExecution) {
    EXPECT_NO_THROW_WITH_MESSAGE({
        ...
        SAFE_PRINT_LINE("[Этап 1] ✅ Primary image: " 
                       + SAFE_TO_STRING(primaryImage.width) + "x" 
        ...
        upscaledImage.width = static_cast<uint32_t>(denoisedImage.width * upscaledImage.scaleFactor);
        upscaledImage.height = static_cast<uint32_t>(denoisedImage.height * upscaledImage.scaleFactor);
        ...
        SAFE_PRINT_LINE("[Этап 4] ✅ Upscaling: " 
                       + SAFE_TO_STRING(denoisedImage.width) + "x" 
...
TEST_F(FullPipelineIntegrationTest, MultiFramePipelinePerformance) {
    ...
    EXPECT_PERFORMANCE_UNDER({
        for (int frame = 0; frame < frameCount; ++frame) {
            ...
            struct { uint32_t w, h; } img = {sceneWidth, sceneHeight};
            (void)img;
        }
...
TEST_F(FullPipelineIntegrationTest, PipelineDifferentResolutions) {
    ...
    for (const auto& res : resolutions) {
        EXPECT_NO_THROW_WITH_MESSAGE({
            SAFE_PRINT_LINE("[Pipeline] Тест " + res.name + ": " 
                           + SAFE_TO_STRING(res.width) + "x" + SAFE_TO_STRING(res.height));
            
            // Primary Rasterization
            struct PrimaryImage { uint32_t w, h; } primary;
            primary.w = res.width;
            ...
            // Upscaling 2x
            struct UpscaledImage { uint32_t w, h; } upscaled;
...
TEST_F(FullPipelineIntegrationTest, AdaptiveQualityPipeline) {
    ...
    for (const auto& preset : presets) {
        EXPECT_NO_THROW_WITH_MESSAGE({
            ...
            SAFE_PRINT_LINE("  - Rasterization samples: " + SAFE_TO_STRING(preset.rasterizationSamples));
            SAFE_PRINT_LINE("  - Ray tracing depth: " + SAFE_TO_STRING(preset.rayTracingDepth));
            SAFE_PRINT_LINE("  - Denoising samples: " + SAFE_TO_STRING(preset.denoisingSamples));
            SAFE_PRINT_LINE("  - Upscale factor: " + SAFE_TO_STRING(preset.upscaleFactor) + "x");
...
TEST_F(FullPipelineIntegrationTest, PipelineStressTest) {
    ...
    EXPECT_NO_THROW_WITH_MESSAGE({
        SAFE_PRINT_LINE("[Pipeline] Стресс-тест: " + SAFE_TO_STRING(stressIterations) + " кадров");
        
        for (int i = 0; i < stressIterations; ++i) {
            try {
                // Полный pipeline
                struct { uint32_t w, h; } img = {sceneWidth, sceneHeight};
                struct { void* data; } effects = {reinterpret_cast<void*>(0x1000)};
                struct { void* data; } denoised = {reinterpret_cast<void*>(0x2000)};
                struct { uint32_t w, h; } upscaled = {img.w * 2, img.h * 2};
