...
static void BM_DifferentResolutions(benchmark::State& state) {
    ...
    for (auto _ : state) {
        ...
        for (int i = 0; i < 10; ++i) {
            Engine3D::Math::Matrix4 transform = Engine3D::Math::Matrix4::translation(
                static_cast<float>(i - 5), 0.0f, 0.0f
            );
    ...
    state.counters["Pixels/sec"] = benchmark::Counter(
        static_cast<double>(pixels_per_frame * state.iterations()),
        benchmark::Counter::kIsRate
    );
}
BENCHMARK(BM_DifferentResolutions)->Args({800, 600})->Args({1920, 1080})->Args({2560, 1440})->Args({3840, 2160});
...
BENCHMARK_F(RenderingBenchmarkFixture, BM_CameraManagement)(benchmark::State& state) {
    ...
    for (int i = 0; i < 10; ++i) {
        ...
        camera->setPosition(Engine3D::Math::Vector3(
            static_cast<float>(i), 0.0f, 5.0f
        ));
