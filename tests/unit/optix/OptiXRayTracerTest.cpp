...
TEST_F(OptiXRayTracerTest, BuildBottomLevelAS) {
    EXPECT_NO_THROW_WITH_MESSAGE({
        ...
        SAFE_PRINT_LINE("[OptiXRayTracer] Построение BLAS для " 
                       + SAFE_TO_STRING(triangleCount) + " треугольников");
...
TEST_F(OptiXRayTracerTest, BuildTopLevelAS) {
    EXPECT_NO_THROW_WITH_MESSAGE({
        ...
        SAFE_PRINT_LINE("[OptiXRayTracer] Построение TLAS для " 
                       + SAFE_TO_STRING(instanceCount) + " инстансов");
...
TEST_F(OptiXRayTracerTest, TraceShadows) {
    EXPECT_NO_THROW_WITH_MESSAGE({
        ...
        SAFE_PRINT_LINE("[OptiXRayTracer] Трассировка теней от " 
                       + SAFE_TO_STRING(lightCount) + " источников света");
...
TEST_F(OptiXRayTracerTest, TraceGlobalIllumination) {
    EXPECT_NO_THROW_WITH_MESSAGE({
        ...
        SAFE_PRINT_LINE("[OptiXRayTracer] Global Illumination с " 
                       + SAFE_TO_STRING(bounceCount) + " bounces");
...
TEST_F(OptiXRayTracerTest, TraceAmbientOcclusion) {
    EXPECT_NO_THROW_WITH_MESSAGE({
        ...
        SAFE_PRINT_LINE("[OptiXRayTracer] Ambient Occlusion с " 
                       + SAFE_TO_STRING(aoSamples) + " samples");
...
TEST_F(OptiXRayTracerTest, ASBuildPerformance) {
    EXPECT_PERFORMANCE_UNDER({
        ...
        SAFE_PRINT_LINE("[OptiXRayTracer] Построение AS для " 
                       + SAFE_TO_STRING(triangleCount) + " треугольников");
...
TEST_F(OptiXRayTracerTest, ValidateGeometry) {
    EXPECT_NO_THROW_WITH_MESSAGE({
        ...
        SAFE_PRINT_LINE("[OptiXRayTracer] Геометрия валидна: " 
                       + SAFE_TO_STRING(triangleCount) + " треугольников");
    }, "Валидация геометрии");
}
...
TEST_P(OptiXRayTracerResolutionTest, DifferentResolutionsAndDepth) {
    ...
    EXPECT_NO_THROW_WITH_MESSAGE({
        ...
        SAFE_PRINT_LINE("[OptiXRayTracer] Трассировка " 
                       + SAFE_TO_STRING(width) + "x" + SAFE_TO_STRING(height) 
                       + " с глубиной " + SAFE_TO_STRING(maxDepth));
...
INSTANTIATE_TEST_SUITE_P(
    ResolutionAndDepthTests,
    OptiXRayTracerResolutionTest,
    ::testing::Values(
        std::make_tuple(1920, 1080, 2),   // Full HD, shallow
