...
TEST_F(FlashGSSplatterTest, RasterizeLargeGaussianSet) {
    ...
    EXPECT_NO_THROW_WITH_MESSAGE({
        ...
        for (size_t i = 0; i < largeSet.size(); ++i) {
            largeSet[i].position = make_float4(
                (float)(i % 100) - 50.0f,
                (float)(i / 100) - 50.0f,
                -10.0f,
                1.0f
        ...
        SAFE_PRINT_LINE("Создан большой набор: " + SAFE_TO_STRING(largeSet.size()) + " гауссианов");
...
TEST_F(FlashGSSplatterTest, ValidateGaussianParameters) {
    EXPECT_NO_THROW_WITH_MESSAGE({
        for (const auto& gaussian : testGaussians) {
            ...
            float qLength = std::sqrt(
                gaussian.rotation.x * gaussian.rotation.x +
...
class FlashGSSplatterResolutionTest 
    : public FlashGSSplatterTest,
...
INSTANTIATE_TEST_SUITE_P(
    ...
    ::testing::Values(
        std::make_tuple(1920, 1080),  // Full HD
