...
TEST_F(ResourceManagerTest, AllocateVertexBuffer) {
    EXPECT_NO_THROW_WITH_MESSAGE({
        ...
        SAFE_PRINT_LINE("[ResourceManager] Vertex buffer для " 
                       + SAFE_TO_STRING(vertexCount) + " вершин");
...
TEST_F(ResourceManagerTest, AllocateIndexBuffer) {
    EXPECT_NO_THROW_WITH_MESSAGE({
        ...
        SAFE_PRINT_LINE("[ResourceManager] Index buffer для " 
                       + SAFE_TO_STRING(indexCount) + " индексов");
...
TEST_F(ResourceManagerTest, AllocateBasicImage) {
    EXPECT_NO_THROW_WITH_MESSAGE({
        ...
        SAFE_PRINT_LINE("[ResourceManager] Создание изображения " 
...
TEST_F(ResourceManagerTest, WriteToMappedMemory) {
    EXPECT_NO_THROW_WITH_MESSAGE({
        ...
        SAFE_PRINT_LINE("[ResourceManager] Запись " 
                       + SAFE_TO_STRING(testData.size()) + " элементов в маппированную память");
...
TEST_F(ResourceManagerTest, CopyBufferToBuffer) {
    EXPECT_NO_THROW_WITH_MESSAGE({
        ...
        SAFE_PRINT_LINE("[ResourceManager] Копирование " 
                       + SAFE_TO_STRING(copySize) + " bytes между буферами");
...
TEST_F(ResourceManagerTest, HandleOutOfMemory) {
    EXPECT_NO_THROW_WITH_MESSAGE({
        ...
        SAFE_ERROR("[ResourceManager] Попытка аллокации " 
                  + SAFE_TO_STRING(hugeSize / (1024*1024*1024)) + " GB - ожидается ошибка");
...
class ResourceManagerBufferSizeTest 
