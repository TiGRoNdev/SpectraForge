...
class VulkanMockFactory {
  public:
    ...
    static std::unique_ptr<MockVulkanRenderer> createInitializedRenderer() {
        ...
        EXPECT_CALL(*mock, init(testing::_, testing::_)).WillOnce(testing::Return(true));
    ...
    static std::unique_ptr<MockVulkanRenderer> createFailingRenderer() {
        ...
        EXPECT_CALL(*mock, init(testing::_, testing::_)).WillRepeatedly(testing::Return(false));
    ...
    static std::unique_ptr<MockResourceManager> createResourceManager() {
        ...
        EXPECT_CALL(*mock, getAllocatedMemory()).WillRepeatedly(testing::Return(0));
    ...
    static std::unique_ptr<MockFlashGSSplatter> createFlashGSSplatter() {
        ...
        EXPECT_CALL(*mock, initialize(testing::_)).WillRepeatedly(testing::Return(true));
    ...
    static std::unique_ptr<MockOptiXRayTracer> createOptiXRayTracer() {
        ...
        EXPECT_CALL(*mock, traceRays(testing::_)).WillRepeatedly(testing::Return(mockEffects));
  ...
  private:
    ...
    static void setupBasicRenderingExpectations(MockVulkanRenderer& mock) {
        ...
        EXPECT_CALL(mock, denoiseAI(testing::_)).WillRepeatedly(testing::Return(mockDenoised));
        ...
        // Презентация
        EXPECT_CALL(mock, presentFinalImage()).Times(testing::AnyNumber());

        // Завершение работы
        EXPECT_CALL(mock, shutdown()).Times(testing::AtMost(1));
    }
};
