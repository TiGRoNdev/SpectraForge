...
public:
    ...
    // Рендеринг объектов
    MOCK_METHOD(void, renderMesh, 
                (const Mesh3D& mesh, const Engine3D::Math::Matrix4& transform, const Shader3D& shader), 
                (override));
    MOCK_METHOD(void, renderMesh, 
                (std::shared_ptr<Mesh3D> mesh, const Engine3D::Math::Matrix4& transform, std::shared_ptr<Shader3D> shader), 
                (override));
    MOCK_METHOD(void, renderWireframe, 
                (const Mesh3D& mesh, const Engine3D::Math::Matrix4& transform, const Shader3D& shader), 
                (override));
...
public:
    MOCK_METHOD(std::shared_ptr<Engine3D::GameObject3D>, createPrimitive, (const std::string& type), (override));
};
...
public:
    ...
    MOCK_METHOD(Engine3D::Math::Vector3, project, (const Engine3D::Math::Vector3& point), (const override));
};
...
public:
    ...
    static std::unique_ptr<MockRendererAdapter> createBasicRenderer() {
        ...
        EXPECT_CALL(*mock, isInitialized())
            .WillRepeatedly(testing::Return(true));
        EXPECT_CALL(*mock, getBackendType())
            .WillRepeatedly(testing::Return(RenderBackend::OPENGL));
        EXPECT_CALL(*mock, getBackendName())
            .WillRepeatedly(testing::Return("MockRenderer"));
    ...
    static std::unique_ptr<MockTransformable> createBasicTransformable() {
        ...
        EXPECT_CALL(*mock, getPosition())
            .WillRepeatedly(testing::Return(defaultPosition));
        EXPECT_CALL(*mock, getTransformMatrix())
            .WillRepeatedly(testing::Return(identityMatrix));
        EXPECT_CALL(*mock, setPosition(testing::_))
