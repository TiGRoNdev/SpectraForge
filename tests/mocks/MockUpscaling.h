...
public:
    ...
    MOCK_METHOD(FinalImage, upscaleImage, 
                (const DenoisedImage& image, const ResolutionTarget& target), (override));
...
public:
    ...
    std::string getUpscalerName() const override {
        return "DLSS";
    }
};
...
public:
    ...
    MOCK_METHOD(QualityMode, recommendQualityMode, (uint32_t targetWidth, uint32_t targetHeight), ());
};
...
public:
    ...
    static std::unique_ptr<MockDLSSUpscaler> createDLSSUpscaler() {
        ...
        EXPECT_CALL(*mock, isInitialized())
            .WillRepeatedly(testing::Return(true));
        EXPECT_CALL(*mock, isSupported())
            .WillRepeatedly(testing::Return(true));
        EXPECT_CALL(*mock, getPerformanceGain())
            .WillRepeatedly(testing::Return(2.5f)); // 2.5x performance gain
        
        // DLSS специфичные ожидания
        EXPECT_CALL(*mock, supportsTensorCores())
            .WillRepeatedly(testing::Return(true));
        EXPECT_CALL(*mock, getDLSSVersion())
            .WillRepeatedly(testing::Return("3.1.1"));
        EXPECT_CALL(*mock, supportsRayReconstruction())
            .WillRepeatedly(testing::Return(true));
        EXPECT_CALL(*mock, supportsFrameGeneration())
            .WillRepeatedly(testing::Return(true));
    ...
    static std::unique_ptr<MockFSRUpscaler> createFSRUpscaler() {
        ...
        EXPECT_CALL(*mock, supportsAsyncCompute())
            .WillRepeatedly(testing::Return(true));
        EXPECT_CALL(*mock, getFSRVersion())
    ...
    static std::unique_ptr<MockHardwareDetector> createHardwareDetector(
        ...
        EXPECT_CALL(*mock, detectVendor())
            .WillRepeatedly(testing::Return(vendor));
        
        if (vendor == HardwareConfig::VendorType::NVIDIA) {
            EXPECT_CALL(*mock, supportsTensorCores())
                .WillRepeatedly(testing::Return(true));
            EXPECT_CALL(*mock, selectOptimalUpscaler())
                .WillRepeatedly(testing::Return("DLSS"));
            EXPECT_CALL(*mock, getDeviceName())
                .WillRepeatedly(testing::Return("RTX 4080"));
            ...
            EXPECT_CALL(*mock, supportsAsyncCompute())
                .WillRepeatedly(testing::Return(true));
            EXPECT_CALL(*mock, selectOptimalUpscaler())
                .WillRepeatedly(testing::Return("FSR"));
            EXPECT_CALL(*mock, getDeviceName())
                .WillRepeatedly(testing::Return("RX 7800 XT"));
        }
        
        EXPECT_CALL(*mock, supportsRayTracing())
            .WillRepeatedly(testing::Return(true));
    ...
    static std::unique_ptr<MockUpscaler> createUnsupportedUpscaler() {
        ...
        EXPECT_CALL(*mock, isSupported())
            .WillRepeatedly(testing::Return(false));
        EXPECT_CALL(*mock, getUpscalerName())
            .WillRepeatedly(testing::Return("Unsupported"));
...
private:
    ...
    static void setupUpscalingResult(MockType& mock) {
        EXPECT_CALL(mock, upscaleImage(testing::_, testing::_))
            .WillRepeatedly(testing::Invoke([](const DenoisedImage& input, const ResolutionTarget& target) {
                FinalImage result;
                ...
                std::fill_n(result.colorBuffer, bufferSize, 0.5f); // Заполняем серым цветом для тестов
