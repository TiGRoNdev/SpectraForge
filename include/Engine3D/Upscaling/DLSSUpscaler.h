...
public:
    ...
    void accelerateInference(const float* input, float* output, 
...
public:
    ...
    void setDLSSOptions(uint32_t outputWidth, uint32_t outputHeight,
...
public:
     ...
     */
    FinalImage upscaleImage(const DenoisedImage& image, 
                           const ResolutionTarget& target) override;
     ...
     */
    static std::pair<uint32_t, uint32_t> getOptimalRenderResolution(
        uint32_t targetWidth, uint32_t targetHeight, const std::string& quality);
