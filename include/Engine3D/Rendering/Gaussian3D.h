...
public:
    ...
    static std::shared_ptr<GaussianField3D> createFromMesh(const class Mesh3D& mesh, int gaussianDensity = 1000);
    static std::shared_ptr<GaussianField3D> createFromPointCloud(const std::vector<Math::Vector3>& points, 
    ...
    struct RenderStats {
        ...
        void reset() { totalGaussians = renderedGaussians = culledGaussians = 0; frameTime = 0.0f; }
    };
