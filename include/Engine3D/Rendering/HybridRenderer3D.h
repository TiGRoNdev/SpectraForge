...
protected:
    ...
    void executeGaussianRasterization(const std::vector<std::shared_ptr<GaussianField3D>>& gaussianFields);
...
public:
    ...
    struct DenoiserStats {
        ...
        void reset() {
            denoiseTime = noiseReduction = detailPreservation = 0.0f;
        }
    };
...
private:
    ...
    void applyEdgeAwareFilter(unsigned int input, unsigned int output,
                             unsigned int normalBuffer);
    void applyTemporalAccumulation(unsigned int input, unsigned int output,
                                  unsigned int motionBuffer);
};
...
public:
    ...
    struct UpscalerStats {
        ...
        void reset() {
            upscaleTime = qualityMetric = 0.0f;
        }
    };
...
private:
    ...
    void applyBilinearUpscaling(unsigned int input, unsigned int output,
                               int inW, int inH, int outW, int outH);
    void applyLanczosUpscaling(unsigned int input, unsigned int output,
                              int inW, int inH, int outW, int outH);
    void applyEdgeEnhancedUpscaling(unsigned int input, unsigned int output,
                                   int inW, int inH, int outW, int outH);
};
