...
namespace SpectraForge::Rendering::Upscaling {
...
class IUpscalingStrategy {
  public:
     ...
     */
    virtual std::pair<int, int> getRecommendedRenderResolution(int targetWidth,
                                                               int targetHeight,
                                                               UpscalingQuality quality) const = 0;
...
struct UpscalingStats {
    ...
    float getFpsBoost(float nativeFrameTime) const {
        if (averageExecutionTime <= 0.0f || nativeFrameTime <= 0.0f)
            return 1.0f;
