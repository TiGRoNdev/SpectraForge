 ...
 */
__global__ void assignGaussiansToTiles(
    const ProjectedGaussian* projected,
 ...
 */
__global__ void sortGaussiansByDepth(
    const ProjectedGaussian* projected,
 ...
 * @param screenHeight Высота экрана
 */
__global__ void rasterizeTiles(
    const ProjectedGaussian* projected,
    ...
    if (globalX >= screenWidth || globalY >= screenHeight) return;
...
extern "C" {
    ...
    cudaError_t launchTileAssignment(
        const ProjectedGaussian* d_projected,
    ...
    cudaError_t launchTileRasterization(
        ...
        dim3 gridSize(
            (screenWidth + TILE_SIZE - 1) / TILE_SIZE,
            (screenHeight + TILE_SIZE - 1) / TILE_SIZE
        );
        
        rasterizeTiles<<<gridSize, blockSize, 0, stream>>>(
            d_projected, d_sortedIndices, d_tileOffsets, d_framebuffer, d_depthBuffer,
